...
def _build_ffmpeg_command(self, input_path, output_path, settings, video_info):
    ...
    视频参数:
        ...
        -b:v {bitrate}        # 码率 (码率模式)
        -maxrate {bitrate}    # 最大码率限制 (码率模式)
        -bufsize {bitrate}    # 缓冲区大小 (码率模式)
        -preset {speed}       # 编码速度
        -vf scale={w}:{h}     # 缩放 (如需要)
        -r {fps}              # 帧率 (如需要)
...
# 指定目标码率 (按大小/按比例模式，含码率限制)
ffmpeg -y -i input.mp4 -c:v libx264 -b:v 2000k -maxrate 2000k -bufsize 2000k -preset medium -c:a aac -b:a 192k output.mp4
